...
class DatabaseRowsUpdateWizard implements UpgradeWizardInterface, RepeatableInterface
{
    ...
    public function executeUpdate(): bool
    {
            ...
            while ($row = $rowBefore = $statement->fetch()) {
                ...
                if (empty($updatedFields)) {
                    ...
                    if ($rowCountWithoutUpdate >= 200) {
                        ...
                        $registry->set('installUpdateRows95', 'rowUpdatePosition95', $startPosition);
                        $rowCountWithoutUpdate = 0;
                    }
                } else {
                    $rowCountWithoutUpdate = 0;
        ...
        $registry->remove('installUpdateRows95', 'rowUpdatePosition95');
    ...
    {
        $doneRowUpdater = GeneralUtility::makeInstance(Registry::class)->get('installUpdateRows95', 'rowUpdatersDone95', []);
    ...
    {
        $registry = GeneralUtility::makeInstance(Registry::class);
        $doneRowUpdater = $registry->get('installUpdateRows95', 'rowUpdatersDone95', []);
        $doneRowUpdater[] = get_class($updater);
        $registry->set('installUpdateRows95', 'rowUpdatersDone95', $doneRowUpdater);
    }
    ...
    {
        $registry = GeneralUtility::makeInstance(Registry::class);
        $startPosition = $registry->get('installUpdateRows95', 'rowUpdatePosition95', []);
    ...
    protected function updateOrDeleteRow(Connection $connectionForTable, Connection $connectionForSysRegistry, string $table, int $uid, array $updatedFields, array $startPosition): void
    {
        ...
        $connectionForSysRegistry->update(
            ...
            [
                'entry_namespace' => 'installUpdateRows95',
                'entry_key' => 'rowUpdatePosition95',
            ],
            [
